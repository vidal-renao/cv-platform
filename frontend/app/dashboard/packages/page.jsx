    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900">{isEdit ? t('packages.modal.editTitle') : t('packages.modal.newTitle')}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('packages.modal.trackingLabel')}</label>
            <input
              required
              ...
              placeholder={t('packages.modal.trackingPlaceholder')}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('packages.modal.clientLabel')}</label>
            <select
              required
            ...
            >
              <option value="">{t('packages.modal.clientPlaceholder')}</option>
              {clients.map((c) => (
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('packages.modal.weightLabel')}</label>
            <div className="relative">
              <input
                ...
                placeholder={t('packages.modal.weightPlaceholder')}
                value={form.weight}
                onChange={(e) => setForm({ ...form, weight: e.target.value })}
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">kg</span>
            </div>
            <p className="mt-1 text-xs text-gray-400">{t('packages.modal.weightHint')}</p>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('packages.modal.descLabel')}</label>
            <input
              className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition text-sm"
              placeholder={t('packages.modal.descPlaceholder')}
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {saving ? t('packages.modal.saving') : isEdit ? t('packages.modal.saveBtn') : t('packages.modal.createBtn')}
            </button>
            <button
              type="button"
              onClick={onClose}
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl font-bold text-gray-900 tracking-tight">{t('packages.title')}</h1>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50/50">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('packages.table.trackingId')}</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('packages.table.client')}</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('packages.table.status')}</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('packages.table.weightCost')}</th>
                <th className="px-6 py-4 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('packages.table.actions')}</th>
              </tr>
            </thead>
                ...
                <tr key={pkg.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                    ...
                    {pkg.status !== 'PICKED_UP' && (
                      ...
                      >
                        {pickingUp === pkg.id ? t('packages.pickupSaving') : t('packages.pickup')}
                      </button>
                    )}
                    <button
