...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const db = getDb();
    const pkgCheck = await db.query('SELECT id FROM packages WHERE id = $1 AND client_id = $2', [id, client.id]);
    if (pkgCheck.rows.length === 0) {
      return NextResponse.json({ error: 'Package not found' }, { status: 404 });
    }

    const result = await db.query('SELECT * FROM delivery_proofs WHERE package_id = $1', [id]);
    if (result.rows.length === 0) {
      return NextResponse.json({ error: 'No proof of delivery yet' }, { status: 404 });
    }
    return NextResponse.json(result.rows[0]);
  } catch (err) {
    console.error('[CLIENT_PACKAGE_PROOF_GET_ERROR]:', err);
